...
int josephus_destroy(Josephus *self){
    array_person_clear(self->people);
    free(self);
    return SUCCESS;
}
...
int josephus_append(Josephus *self, Person *someone){
    if(array_person_size(self->people) == self->capacity){
        return FAILURE;
    }
...
Person *josephus_pop(Josephus *self, int pop_number){
    if(array_person_size(self->people) <= 0){
        return FAILURE;
    }

    Person **pop_person;
    array_person_pop_at(pop_person, self->people, pop_number);
...
int josephus_query(Josephus *self){
    ...
    for(int i = 0; i < array_person_size(self->people); i++){
        Person *someone = *array_person_get(self->people, i);
...
Person *josephus_output_next(Josephus *self){
    int next_number = (self->start + self->step + self->_current - 2) % array_person_size(self->people);
